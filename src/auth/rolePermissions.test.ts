...
describe('rolePermissions', () => {
  describe('getOrgNavItems', () => {
    ...
    it('returns 4 items for MANAGER including Approvals', () => {
      ...
      expect(items.map((item) => item.label)).not.toContain('Organization Settings')
    })

    it('[P0] returns 8 items for ORGANIZATION_ADMIN including Reports, Data Import, Balance Corrections and Organization Settings', () => {
      const items = getOrgNavItems('ORGANIZATION_ADMIN')
      // My settings sits directly above Organization Settings so the two Settings
      // destinations read as a pair at the foot of the nav.
      expect(items.map((item) => item.label)).toEqual([
        'Team Calendar',
        'My Leaves',
        'Approvals',
    ...
    it('[P0] hides Reports navigation when the plan does not entitle reporting', () => {
      ...
      // an un-gated nav item would send every Organization admin to a denial banner.
      const items = getOrgNavItems('ORGANIZATION_ADMIN', true, false)
      expect(items.map((item) => item.label)).not.toContain('Reports')
      expect(items.map((item) => item.label)).toContain('Organization Settings')
      expect(getOrgNavItems('ORGANIZATION_ADMIN', true, true).map((item) => item.label))
        .toContain('Reports')
    })
    ...
    it('[P0] every nav testId resolves to a layout:nav key in both locales', () => {
      ...
      for (const item of getOrgNavItems('ORGANIZATION_ADMIN')) {
  ...
  describe('canAccessOrgRoute', () => {
    ...
    it('allows approvals for managers, Organization admins, and assigned employees only', () => {
      ...
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/approvals')).toBe(true)
    })

    it('restricts settings to Organization admin only', () => {
      expect(canAccessOrgRoute('EMPLOYEE', '/settings')).toBe(false)
      expect(canAccessOrgRoute('MANAGER', '/settings')).toBe(false)
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/settings')).toBe(true)
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/settings/team')).toBe(true)
    })

    it('[P0] restricts reports to Organization admin only', () => {
      expect(canAccessOrgRoute('EMPLOYEE', '/reports')).toBe(false)
      expect(canAccessOrgRoute('MANAGER', '/reports')).toBe(false)
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/reports')).toBe(true)
    })

    it('[P0] restricts data import to Organization admin only, mirroring /settings', () => {
      expect(canAccessOrgRoute('EMPLOYEE', '/import')).toBe(false)
      expect(canAccessOrgRoute('MANAGER', '/import')).toBe(false)
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/import')).toBe(true)
    })

    it('[P0] restricts balance corrections to Organization admin only, mirroring /settings', () => {
      expect(canAccessOrgRoute('EMPLOYEE', '/corrections')).toBe(false)
      expect(canAccessOrgRoute('MANAGER', '/corrections')).toBe(false)
      expect(canAccessOrgRoute('ORGANIZATION_ADMIN', '/corrections')).toBe(true)
    })
  })
  ...
  describe('home redirects', () => {
    it('returns the team calendar for org roles', () => {
      ...
      expect(getHomePath('ORGANIZATION_ADMIN')).toBe('/calendar')
    })
